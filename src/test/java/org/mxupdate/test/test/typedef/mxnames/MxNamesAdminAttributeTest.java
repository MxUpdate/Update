 ...
 */
public class MxNamesAdminAttributeTest
    extends AbstractTest
{
        ...
        throws Exception
    {
        ...
        final SortedSet<String> tmp = paramCache.getMapping().getTypeDef(CI.DM_ATTRIBUTE.updateType).matchMxNames(paramCache, Arrays.asList(new String[]{AbstractTest.PREFIX + "*"}));

        Assert.assertEquals(tmp, new TreeSet<>(Arrays.asList(attr.getName())));
    }
        ...
        throws Exception
    {
        ...
        MqlBuilderUtil_mxJPO.mql().cmd("escape add attribute ").arg(AbstractTest.PREFIX + "MyPathType").cmd(" type string owner type ").arg(type.getName()).exec(this.getContext());
